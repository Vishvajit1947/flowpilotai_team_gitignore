...
## Objective
Create the complete project README file (`README.md` in the repository root) that explains what FlowPilot AI is, how to set it up locally, how to run it with Docker, the project architecture, API reference summary, contributing guidelines, and deployment instructions.
...
- Internal engineering docs (this docs/ folder)

## Functional Requirements
1. A developer can clone the repo and run the project locally in under 15 minutes by following the README.
2. All required environment variables are listed with descriptions.
3. Architecture overview clearly explains the tech stack.
4. Commands are copy-pasteable and tested.

## Technical Requirements
- Markdown format
- GitHub-flavored Markdown
- Mermaid diagram for architecture (GitHub renders these)

## Folder Structure
```
FlowPilot-AI/
└── README.md    # THE FILE TO CREATE
```
...
- **Analytics Dashboard** — Real-time submission stats, agent performance, daily trends
...
None — this is a new root-level file.

## API Contracts
Documents existing API contracts.

## Request Examples
All setup commands are copy-pasteable.
...
## Business Logic
- README is the single source of truth for "how to run this project".
- All commands tested on macOS/Linux; Windows variations noted where necessary.
- Architecture diagram uses Mermaid (renders in GitHub).

## Validation Rules
- All commands in README must work on a fresh clone.
- Environment variable table must be complete.
...
## Edge Cases
- Windows users: `source .venv/bin/activate` → `.venv\Scripts\activate`
- Apple Silicon: all dependencies are ARM64-compatible
- Tesseract path: on some systems, `pytesseract.pytesseract.tesseract_cmd` may need to be set explicitly

## Test Cases
1. Following README from scratch produces a working local environment.
2. `alembic upgrade head` succeeds against a fresh Supabase database.
3. Docker Compose starts both services successfully.
4. API docs available at `http://localhost:8000/docs`.

## Acceptance Criteria
- [ ] README covers setup, Docker, env vars, API reference, deployment
- [ ] All commands are tested and copy-pasteable
- [ ] Mermaid architecture diagram renders in GitHub
- [ ] Prerequisites listed clearly (Tesseract, Poppler, Node, Python)
- [ ] License section present

## Definition of Done
- README at repository root
- Tested by a developer who has not seen the codebase before
